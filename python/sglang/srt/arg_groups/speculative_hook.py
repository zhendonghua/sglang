...
def _handle_decoupled_spec(server_args: ServerArgs) -> None:
    ...
    # Transport endpoints are required (the ZMQ mesh has no addresses otherwise).
    # PortArgs.init_new raises the same later; this surfaces it earlier.
    if (
    ...
    # paged rollback is not implemented. This is a drafter simplification, not a
    # kernel limit (topk==1 supports paging). To support page_size > 1, add
    # page-aligned drafter rollback. None means unresolved, treated as ok.
    ...
    # Synchronous scheduler: the cross-process verify/commit handshake must observe
    # each step before starting the next, which the overlap pipeline would break.
    ...
    # Drafter only.
    # - Radix cache off: token-granular rollback frees a diverged tail, which
    #   prefix sharing / COW complicates. Relaxable later — the speculative tail is
    #   never inserted into the tree, so rollback can reuse the colocated
    #   overallocated-free path under the existing lock_ref.
    # - Mamba no_buffer: the extra_buffer inter-request state-donation path is not
    #   validated for decoupled coordination yet, so use the snapshot mode. Set
    #   explicitly because the mamba handler already ran earlier in __post_init__,
    #   so disabling radix here does not undo it. (Hybrid recurrent-state rollback
    ...
    # Decoupled runs a real draft algorithm: it must be named, and num_steps (the
    # speculation depth that sets num_draft_tokens and the drafter ahead window)
    # must be set.
    ...
    # topk == 1 (chain only): the IPC protocol streams linear tokens, so tree
    # drafting (topk > 1) cannot be expressed on the wire yet. Not a kernel limit.
    # To support it, extend the draft-tail protocol with tree structure + tree
    # verify. Pin to 1 so downstream reads a concrete value.
    if (
