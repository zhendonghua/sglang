...
class TestDecoupledSpecHookEnforce(CustomTestCase):
    ...
    def test_pp_size_gt_1_raises(self):
        # The cross-process handshake is unvalidated against the PP microbatch
        # loop; decoupled requires pp_size == 1.
        with self.assertRaises(ValueError):
            handle_decoupled_spec(_StubArgs(pp_size=2))

    def test_fanout_below_one_raises(self):
        with self.assertRaises(ValueError):
            handle_decoupled_spec(_StubArgs(speculative_fanout=0))
...
class TestDecoupledSpecHookNormalizeAndForce(CustomTestCase):
    ...
    def test_fanout_defaulted_to_four(self):
        # F sizes the drafter's enumeration buffer (B x (K+1) x F x K); unset
        # resolves to the role default so both engines agree on the layout.
        args = _StubArgs(speculative_fanout=None)
        handle_decoupled_spec(args)
        self.assertEqual(args.speculative_fanout, 4)

    def test_explicit_fanout_kept(self):
        args = _StubArgs(speculative_fanout=8)
        handle_decoupled_spec(args)
        self.assertEqual(args.speculative_fanout, 8)
